...
The longest movie in this data set is the 'Cure for Insomnia' with a run time of 5220 minutes.    
The oldest movie in this data set is 'Blacksmith Scene' filmed in 1893.   
...
1248 Movies were released in 1995.   
...
These 10 comedies are the only ones with a length over 120 minutes, an MPAA rating, and a rating of 8 or higher.   
...
```{r}
harry_potter_movies <- movies %>%
  select(title, rating, budget, votes) 
harry_potter_movies_filtered <- harry_potter_movies %>%
  filter(str_detect(title, "Harry Potter"))
harry_potter_movies_filtered
```

These 'Harry Potter' movies are included this data subset:   
Harry Potter and the Chamber of Secrets	   
Harry Potter and the Prisoner of Azkaban	  
Harry Potter and the Sorcerer's Stone   

'Harry Potter and the Prisoner of Azkaban' is the highest rated with a rating of 7.8.    

'Harry Potter and the Sorcerer's Stone' has the most votes on IMDB with 46741.    

4. What is the highest budget for a movie that is an Animation or Comedy with at least 100 votes?   

```{r}
movies_comedies_animation_100 <- movies %>%
    filter(Comedy == 1 | Animation == 1 & votes >= 100)

movies_comedies_animation_100 %>%
  arrange(desc(budget))
```

'Wild Wild West' is the highest budget comedy or animation movie with at least 100 votes on imdb, it is a comedy.    

5. Is your favorite movie in the dataset? What other questions could you ask or answer using `filter()`, `select()`, and `arrange()`?

```{r}
movies %>%
  filter(str_detect(title, "Wolf of Wall Street"))
```

My favorite movie 'Wolf of Wall Street' is not included in this data set, potentially because it's a controversial film.    

You could answer questions like what is the highest budget film in each genre, or what is the film with the largest number of votes on IMDB with a rating below 3.0.   
